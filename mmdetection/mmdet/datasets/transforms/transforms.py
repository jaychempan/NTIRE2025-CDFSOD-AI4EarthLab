...
class CopyPaste(BaseTransform):
    ...
    def _get_updated_masks(self, masks: BitmapMasks,
        ...
        print(masks.masks.shape[-2:], composed_mask.shape[-2:])
...
class RandomErasing(BaseTransform):
    ...
    Args:
        n_patches (int | tuple[int, int]): 擦除区域的数量或范围。
           - 如果是整数,固定擦除该数量的区域
           - 如果是元组,则在[n_patches[0], n_patches[1]]范围内随机选择数量
        ratio (float): 擦除区域的比例
        img_border_value (int): 填充值
        bbox_erased_thr (float): 框被擦除的阈值
    """

    def __init__(self,
                 n_patches=(1, 3),  # 修改默认值为元组
                 ratio=0.3,
    ...
    def _get_n_patches(self):
        ...
        if isinstance(self.n_patches, int):
            ...
            return np.random.randint(self.n_patches[0], self.n_patches[1] + 1)

    def _get_patches(self, img_shape):
        """生成擦除区域。"""
        h, w = img_shape
        patches = []
        ...
        for _ in range(n_patches):
            # 计算擦除区域的大小
            area = h * w * self.ratio
            target_area = random.uniform(0.02, self.ratio) * area
            aspect_ratio = random.uniform(0.3, 1/0.3)
            
            ph = int(round(math.sqrt(target_area * aspect_ratio)))
            pw = int(round(math.sqrt(target_area / aspect_ratio)))
            
            if pw < w and ph < h:
                px1 = random.randint(0, w - pw)
                py1 = random.randint(0, h - ph)
                patches.append([px1, py1, px1 + pw, py1 + ph])
        
        return np.array(patches)

    def _transform_img(self, results, patches):
        """擦除图像区域。"""
        for patch in patches:
            x1, y1, x2, y2 = patch
            results['img'][y1:y2, x1:x2] = self.img_border_value

    def _transform_bboxes(self, results, patches):
        """处理边界框。"""
        if len(patches) == 0:
            return
            
        bboxes = results['gt_bboxes'].tensor.numpy()  # 获取边界框的numpy数组
        ...
        for bbox in bboxes:
            ...
            for patch in patches:
                # 计算交集区域
                x1 = max(bbox[0], patch[0])
                y1 = max(bbox[1], patch[1])
                x2 = min(bbox[2], patch[2])
                y2 = min(bbox[3], patch[3])
                
                if x2 > x1 and y2 > y1:
                    inter_area = (x2 - x1) * (y2 - y1)
                    patch_iou = inter_area / bbox_area
                else:
                    patch_iou = 0
                    
                patch_ious.append(patch_iou)
            
            # 使用最大的IoU
            box_patches_ious.append(max(patch_ious))
            
        box_patches_ious = np.array(box_patches_ious)
        valid_inds = box_patches_ious < self.bbox_erased_thr
    ...
    def transform(self, results):
        ...
        patches = self._get_patches(results['img'].shape[:2])
